...
local opts = {
    ...
    --reset yt-dlp format to the original format string when changing files (e.g. going to the next playlist entry)
...
function show_menu(isvideo)
    ...
    if options[1] ~= nil then
        mp.add_forced_key_binding(opts.select_binding, "select", function()
            ...
            mp.set_property("ytdl-raw-options", "") --reset yt-dlp raw options before changing format
            mp.set_property("ytdl-format", format_string(vfmt, afmt))
            reload_resume()
        end)
    end
    mp.add_forced_key_binding(isvideo and opts.toggle_video_menu_binding or opts.toggle_audio_menu_binding, "escape",
        destroy)
    ...
    if json.formats ~= nil then
        for i, f in ipairs(json.formats) do
            if f.vcodec ~= "none" then
                ...
                local l = string.format("%-9s %-5s %9s %9s (%-4s / %s%s)", resolution, fps, tbr, size, f.ext, vcodec,
                    acodec)
                table.insert(vres, { label = l, format = f.format_id, width = f.width, size = f.filesize, fps = f.fps, tbr =
                f.tbr })
